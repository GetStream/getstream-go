...
func TestVerifyAndParseWebhookFromRequest(t *testing.T) {
	secret := "test-webhook-secret"
	...
	if err != nil {
		t.Fatalf("VerifyAndParseWebhook(*http.Request): %v", err)
	}
	if event == nil {
...
func TestVerifyAndParseWebhookFromRequest_NilRequest(t *testing.T) {
	_, err := VerifyAndParseWebhook(nil, "secret")
	if !errors.Is(err, ErrInvalidWebhook) {
		t.Fatalf("expected ErrInvalidWebhook for nil request, got %v", err)
	}
}

func TestWebhookMiddleware_HappyPath(t *testing.T) {
	secret := "test-webhook-secret"
	body := []byte(`{"type":"message.new"}`)
	h := hmac.New(sha256.New, []byte(secret))
	h.Write(body)
	...
	handler := WebhookMiddleware(secret)(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		v := r.Context().Value(WebhookEventKey)
		if v == nil {
	...
	if seen == nil {
		t.Fatal("expected event to be stored in context")
	}
}

func TestWebhookMiddleware_BadSignature(t *testing.T) {
	secret := "test-webhook-secret"
	body := []byte(`{"type":"message.new"}`)
	...
	if called {
		t.Fatal("expected downstream handler not to be invoked")
	}
}

// ---------------------------------------------------------------------------
// Conformance fixture suite (CHA-2961). Generated by chat-manager openapi
// generate-webhook-fixtures and copied into tests/fixtures/webhooks/ at
// codegen time.
// ---------------------------------------------------------------------------

const canonicalTestSecret = "test_secret_do_not_use_in_production"

func fixtureRoot(t *testing.T) string {
	t.Helper()
	// Test file location depends on SDK layout. For getstream-go, webhook_test.go
	// sits at the repo root, so fixtures land at tests/fixtures/webhooks/.
	cwd, err := os.Getwd()
	if err != nil {
		t.Fatal(err)
	}
	return filepath.Join(cwd, "tests", "fixtures", "webhooks")
}
...
func readStr(t *testing.T, p string) string {
	return strings.TrimSpace(string(readFile(t, p)))
}

func happyFixtureDirs(t *testing.T) []string {
	t.Helper()
	root := fixtureRoot(t)
	entries, err := os.ReadDir(root)
	if err != nil {
		if os.IsNotExist(err) {
			t.Skip("conformance fixtures not present (run generate.sh to populate)")
		}
		t.Fatal(err)
	}
	var dirs []string
	for _, e := range entries {
		if e.IsDir() && e.Name() != "_invalid" {
...
func TestWebhookConformance_Happy(t *testing.T) {
	for _, dir := range happyFixtureDirs(t) {
		eventType := filepath.Base(dir)
		t.Run(eventType, func(t *testing.T) {
			body := readFile(t, filepath.Join(dir, "body.json"))
			bodyGz := readFile(t, filepath.Join(dir, "body.gz"))
			sqsCompressed := readStr(t, filepath.Join(dir, "sqs_body.txt"))
			sqsRaw := readStr(t, filepath.Join(dir, "sqs_body_uncompressed.txt"))
			sns := readStr(t, filepath.Join(dir, "sns_notification.txt"))
			sig := readStr(t, filepath.Join(dir, "signature.txt"))

			// verify_signature against body.json
			if !VerifySignature(body, sig, canonicalTestSecret) {
				t.Fatal("VerifySignature returned false on canonical body+signature")
			}

			// ParseEvent on body.json
...
func TestWebhookConformance_Negative(t *testing.T) {
	root := fixtureRoot(t)
	invalidRoot := filepath.Join(root, "_invalid")
	...
	t.Run("unknown_type", func(t *testing.T) {
		body := readFile(t, filepath.Join(invalidRoot, "unknown_type", "body.json"))
		ev, err := ParseEvent(body)
		if err != nil {
			t.Fatalf("expected UnknownEvent, got error: %v", err)
		}
		u, ok := ev.(*UnknownEvent)
		if !ok {
			t.Fatalf("expected *UnknownEvent, got %T", ev)
		}
		if u.Type != "totally.made.up" {
			t.Fatalf("expected type 'totally.made.up', got %q", u.Type)
		}
	})
	...
	expectInvalid("bad_compression", "gzip decompression failed", func() (WebhookEvent, error) {
	...
	// one. Still ErrInvalidWebhook.
	expectInvalid("bad_sns_envelope", "", func() (WebhookEvent, error) {
